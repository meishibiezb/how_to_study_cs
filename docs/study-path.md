...
&nbsp;&nbsp;&nbsp;&nbsp;不过，为什么是“高级”编程语言？难道新手不应该从基础学起，比如“初级”编程语言？
...
&nbsp;&nbsp;&nbsp;&nbsp;我推荐新手学习C语言作为第一门高级编程语言，因为学习它能让你对内存有更深的理解。
尤其是的C语言“指针”。
虽然指针是一个很麻烦并且也不是很安全的东西，但是深刻地理解指针对你的益处是很大的。
学校也有很大概率会在第一个学习开设C语言的课程。
当然，也有人会推荐Python语言作为第一门语言，有些学校也会在第一个学期开设Python课程。
下面是一些常见的高级编程语言的介绍。
如果你不知道该选择哪个，从上面的两者中选择其一，或者干脆跟着学校的课程学习。
:::tip 先看看计算机技术的应用领域
如果你暂时不想了解编程语言，点击[这里](#计算机技术的应用领域)看下一部分。
:::

:::details C
C语言是一门古老而强大的过程式编程语言，以其接近硬件的特性和极高的运行效率著称。它提供了对内存的底层控制，是操作系统、嵌入式系统等性能关键领域的基础。学习C语言能帮助你深入理解计算机如何工作，但需要手动管理内存等细节也使其对初学者有一定挑战。
:::

:::details C++
C++是从C语言发展而来的多范式语言，在保留C的高效和底层控制能力的同时，增加了面向对象、泛型等现代特性。它被广泛应用于游戏开发、高性能服务器、图形处理等需要兼顾复杂抽象和极致性能的领域，但语言本身非常复杂，学习曲线陡峭。
:::

:::details Java
Java以其“一次编写，到处运行”的跨平台能力而闻名，这得益于Java虚拟机（JVM）。它是一门严格的面向对象语言，拥有强大的内存自动管理（垃圾回收）和丰富的生态系统。长期以来，Java一直是大型企业级后端开发、安卓应用开发中最主流的选择之一。
:::

:::details Python
Python以其极度简洁、易读的语法和快速开发能力而受到广泛欢迎，常被推荐为新手的第一门语言。它拥有一个庞大而全面的标准库和第三方库生态系统，在数据科学、人工智能、机器学习、Web后端和自动化脚本等领域占据主导地位，尽管其运行速度通常不如编译型语言。
:::

:::details Rust
Rust是一门现代系统级编程语言，旨在提供C++级别的性能和控制力，同时通过其创新的所有权系统保证内存安全和线程安全，无需垃圾回收。它解决了C/C++中常见的内存错误问题，正在成为系统编程、WebAssembly和关键基础设施领域的新兴力量，但概念新颖，入门有一定难度。
:::

:::details C#
C#是微软主导开发的一种现代、类型安全的面向对象语言，运行在.NET平台上。它语法优雅，功能全面，集成了Windows开发的强大支持，是开发Windows桌面应用、游戏（借助Unity引擎）和企业级后端服务的重要工具。近年来，借助.NET Core，其跨平台能力也得到了极大增强。
:::

:::details JavaScript
JavaScript是Web的三大核心语言之一，最初专为网页交互而生，如今已通过Node.js等环境成为全栈开发的重要力量。它是一门解释型的脚本语言，特点是非常灵活、异步编程模型强大，几乎所有的现代浏览器和Web应用都离不开它，生态极其繁荣。
:::

:::details Lua
Lua是一门轻量级、高效且可嵌入的脚本语言。它设计小巧，速度快，常作为嵌入式脚本被集成到其他大型应用程序中，为宿主程序提供扩展和定制能力，尤其是在游戏领域（如《魔兽世界》的插件）和嵌入式系统中非常流行。
:::

:::details PHP
PHP是一门专为Web开发设计的服务器端脚本语言，曾一度统治互联网后端。它的语法简单，上手快，与HTML融合紧密，拥有巨大的历史存量项目和成熟的框架（如Laravel）。虽然在新项目中的热度有所下降，但仍然是驱动大量网站的重要力量。
:::

:::details Go
Go（又称Golang）是由Google设计的一门静态强类型语言，以其简洁的语法、高效的编译速度和强大的并发原生支持（goroutine）而闻名。它非常适合构建高性能、可扩展的分布式网络服务和命令行工具，在云计算和基础设施领域备受青睐。
:::

:::details Fortran
Fortran是世界上最早的高级编程语言之一，专为科学计算和数值分析设计。至今仍在气象学、流体力学、物理模拟等计算密集型领域保持着强大的生命力，因为经过多年优化，其编译器能生成极其高效的数值计算代码。
:::

:::details Lisp
Lisp是历史最悠久的高级编程语言家族之一，以其独特的完全由括号构成的语法和强大的宏系统著称。它是函数式编程的先驱，尤其在人工智能研究的早期阶段扮演了核心角色。其现代方言如Clojure仍在活跃发展中。
:::

:::details Kotlin
Kotlin是一门运行在JVM上的现代编程语言，由JetBrains开发，现已成为Android官方首选的开发语言。它完全兼容Java，但语法更加简洁安全，提供了空安全、扩展函数等特性，大大减少了代码量和潜在的错误。除了Android开发，Kotlin也支持全栈开发，是一门既实用又优雅的语言。
:::

:::details Swift
Swift是苹果公司推出的现代编程语言，用于开发iOS、macOS、watchOS和tvOS应用程序。它结合了C和Objective-C的优点，语法简洁明了，安全性高，执行性能优秀。Swift在设计时注重开发体验，提供了交互式编程环境，让应用开发变得更加高效和愉快。
:::
...
:::tip 先看看操作系统和计算机组成原理
如果你暂时不想了解计算机技术的应用领域，点击[这里](#操作系统和计算机组成原理)看下一部分。
:::
...
:::details 编译原理
编译原理是计算机科学的核心基础课程，它研究的是如何将高级编程语言编写的源代码（人类可读）转换为计算机硬件能够直接理解和执行的机器码（低级语言）的理论、技术和方法。这个过程主要由称为“编译器”的程序完成。你会学习到编译器是如何一步步工作的，通常包括词法分析、语法分析、语义分析、中间代码生成、代码优化和目标代码生成等阶段。学习这门课不仅能让你明白你写的代码是如何最终变成可执行文件的，更能极大地提升你对程序运行机理的理解深度，帮助你写出更高效、更优质的代码，同时也是从事编程语言设计、开发工具研发等领域的基础。
:::
...
&nbsp;&nbsp;&nbsp;&nbsp;理解了操作系统，你就能明白程序运行时的环境是怎样的。你会理解什么是“系统调用”，为什么有些操作（如直接读写硬盘）不被允许；你会明白“线程”和“进程”的区别以及如何高效使用它们；你会对程序的性能、安全性和稳定性有更深层次的认识。这对于你未来进行系统级编程、后端开发、性能优化至关重要。    
### 计算机组成原理
...
&nbsp;&nbsp;&nbsp;&nbsp;理解了组成原理，你就不再觉得电脑是一个神秘的黑盒。你会明白你写的每一行代码最终是如何被硬件执行的，为什么会存在性能瓶颈（比如著名的“内存墙”问题），以及如何从硬件层面去思考和优化程序。这是你区别于“程序员”和“优秀工程师”的关键之一。  
### 两者的关系
